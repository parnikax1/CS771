...
################################
def my_fit1( X_train, y0_train, y1_train, C_value):
################################
#  Non Editable Region Ending  #
################################

	# Train linear models for Response0 and Response1
	model0 = LinearSVC(max_iter=1000, penalty=C_value)
	model0.fit((my_map(X_train)), y0_train)
	
	W0 = model0.coef_.reshape(-1)
	b0 = model0.intercept_[0]
	model1 = LinearSVC(max_iter=1000, penalty=C_value)
...
def my_fit2( X_train, y0_train, y1_train, C_value ):
################################
#  Non Editable Region Ending  #
################################

	# Train linear models for Response0 and Response1
	model0 = LogisticRegression(solver='liblinear', penalty=C_value)
	model0.fit((my_map(X_train)), y0_train)
	
	W0 = model0.coef_.reshape(-1)
	b0 = model0.intercept_[0]
	model1 = LogisticRegression(solver='liblinear', penalty=C_value)
	model1.fit((my_map(X_train)), y1_train)
	W1 = model1.coef_.reshape(-1)
	b1 = model1.intercept_[0]
	return W0, b0, W1, b1
